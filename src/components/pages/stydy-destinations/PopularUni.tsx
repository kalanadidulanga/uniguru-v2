import { Button } from '@/components/myComponents/button'
import Image from 'next/image'
import Link from 'next/link'
import React from 'react'

interface PopulatCoursesProps {
    dataSet: {
        title: string
        universities: {
            name: string
            link: string
        }[]
    }
}

const PopularUni = ({ dataSet }: PopulatCoursesProps) => {
    return (
        <div className='my-container flex flex-col gap-8 items-center mb-24'>
            <h3 className="text-3xl font-bold text-my-blue text-center lg:text-start">{dataSet?.title}</h3>
            <div className=' flex items-center flex-col lg:flex-row w-full gap-8 lg:gap-10'>
                <div className='w-full lg:w-1/2 flex flex-col items-center justify-center gap-5'>
                    {dataSet?.universities.map((university, index) => (
                        <Link key={index} href={university.link} className='w-full max-w-sm'>
                            <Button size={'xl'} className='w-full max-w-sm' variant={'blue2'}>
                                {university.name}
                            </Button>
                        </Link>
                    ))}
                </div>
                <div className='w-full max-w-xl lg:w-1/2 flex items-center justify-center rounded-lg overflow-hidden'>
                    <Image src={'/images/study_destinations/popularunivercities.jpg'} alt='Popular Courses' width={300} height={300} className=' w-full h-full object-cover object-center' />
                </div>
            </div>
        </div>
    )
}

export default PopularUni